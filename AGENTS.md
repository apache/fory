...
- Read context/read state must not expose ref-publication pause/resume APIs. Nested reference
  publication should be stack-aware inside the read state so immutable, generated, and conversion
  owners can materialize final values without publishing temporary owners.
