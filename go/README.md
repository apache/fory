...
- `fory.GenericSet` (set type)
- `interface{}` (dynamic typing)
- User-defined `struct` types (with `//fory:gen` annotation for codegen)

### Type Constraints
- Pointer to pointer and pointer to interface are not supported
- Anonymous types must be registered explicitly for cross-language compatibility
- Note: Code generation path supports additional unsigned integer types (`uint16`, `uint32`, `uint64`, `uint`) beyond the reflection-based path
