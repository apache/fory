...
// ============================================================================
// float16ArraySerializer - optimized [N]float16 serialization (using [N]uint16 storage)
// ============================================================================
...
func (s float16ArraySerializer) WriteData(ctx *WriteContext, value reflect.Value) {
	// Treat as uint16 for raw data transfer, but use FLOAT16_ARRAY wire type
	// Go doesn't have float16, so we assume storage in uint16
	buf := ctx.Buffer()
	length := value.Len()
	size := length * 2
	buf.WriteLength(size)
