...
func (s byteSliceSerializer) Write(f *Fory, buf *ByteBuffer, value reflect.Value) error {
	...
	case reflect.Slice:
		if t.Elem().Kind() == reflect.Uint8 {
			b := value.Bytes()
			return f.WriteBufferObject(buf, &ByteSliceBufferObject{b})
		}
	case reflect.Array:
		if t.Elem().Kind() == reflect.Uint8 {
			n := t.Len()
			...
			src := arr.Slice(0, n)
			tmp := make([]byte, n)
			reflect.Copy(reflect.ValueOf(tmp), src)
			return f.WriteBufferObject(buf, &ByteSliceBufferObject{tmp})
		}
	}
	return fmt.Errorf("not []byte or [N]byte : %v", t)
}
