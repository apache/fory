 ...
 * <p>
 * This class provides direct access to pooled {@code byte[]} arrays to minimize
 * allocation overhead and GC pressure in high-performance applications.
 * <p>
 * <strong>WARNING:</strong> This is a low-level utility. The consumer is solely
 * responsible for returning the borrowed buffer by calling {@link #release(byte[])}
 * in a {@code finally} block. Failure to do so will result in a buffer leak,
...
public final class BufferPool {
  ...
  /**
   * Defines the pooling strategy (buffer size -> number of buffers).
   * This map is made unmodifiable for safety.
   */
  ...
  /**
   * The core data structure of the pool.
   * Key: The size of the buffer arrays in a tier.
   * Value: A thread-safe queue holding the available buffers for that size.
   */
  private final Map<Integer, ConcurrentLinkedDeque<byte[]>> pool;

  /**
   * An array of the available buffer sizes, sorted in ascending order.
   * This is used for efficient lookup of the appropriate buffer tier.
   */
  ...
  /**
   * Borrows a {@code byte[]} from the pool.
   * <p>
   * This is a non-blocking method. It efficiently finds the smallest available buffer that is
   * greater than or equal to the required size. If no suitable buffer is available, it returns
   * an empty Optional immediately.
   *
   * @param requiredSize The minimum required capacity of the buffer in bytes.
   * @return An {@link Optional} containing a {@code byte[]}, or an empty Optional if the pool is exhausted.
   */
  public byte[] borrow(int requiredSize) {
    // Use binary search to find the smallest suitable buffer tier, which is much
    ...
    // Iterate from the best-fit size upwards to find an available buffer.
    for (int i = startIndex; i < bufferSizes.length; i++) {
      ...
      if (buffer != null) {
        // Found a buffer, return it directly.
        return buffer;
      }
    }
   ...
   * <p>
   * It is critical for the consumer to call this method for every buffer borrowed
   * to prevent pool leaks.
   *
   * @param buffer The byte array to return to the pool. It must be an array that was
   * previously borrowed from this pool.
   */
