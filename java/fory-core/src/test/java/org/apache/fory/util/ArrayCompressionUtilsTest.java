...
public class ArrayCompressionUtilsTest {
  ...
  public void testConsistencyBetweenSIMDAndScalar() {
    ...
    for (int[] array : testArrays) {
      PrimitiveArrayCompressionType result =
          ArrayCompressionUtils.determineIntCompressionType(array);
      assertTrue(
          result == PrimitiveArrayCompressionType.NONE
              || result == PrimitiveArrayCompressionType.INT_TO_BYTE
              || result == PrimitiveArrayCompressionType.INT_TO_SHORT);
    }

    for (long[] array : testLongArrays) {
      PrimitiveArrayCompressionType result =
          ArrayCompressionUtils.determineLongCompressionType(array);
      assertTrue(
          result == PrimitiveArrayCompressionType.NONE
              || result == PrimitiveArrayCompressionType.LONG_TO_INT);
    }
  }
