...
public class Encoders {
  ...
  static void loadMapCodecs(TypeRef<?> type, CodecFormat codecFactory) {
  ...
  private static TypeRef<?> token4BeanLoad(
      Set<TypeRef<?>> set, TypeRef<?> init, CodecFormat codecFactory) {
  ...
  static Class<?> loadOrGenRowCodecClass(Class<?> beanClass, CodecFormat codecFactory) {
    Set<Class<?>> classes =
  ...
  static <B> Class<?> loadOrGenArrayCodecClass(
      TypeRef<? extends Collection<?>> arrayCls, TypeRef<B> elementType, CodecFormat codecFactory) {
