...
`@ForyStruct`. For concrete Fory JSON object models annotated with `@JsonType`, it generates a
`_ForyJsonCodec` companion with direct field, property-method, and creator operations, together with
exact Android R8 rules. The generated rules retain effective `@JsonValue`, `@JsonRawValue`, and
`@JsonBase64` members, their runtime annotations, and selected codec constructors. The outputs are
generated by javac during the application build.
...
component accessors directly. `@JsonCodec` uses ordinary type, field, method, and parameter
