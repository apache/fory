...
`@JsonType`, plus metadata for `@JsonCodec` type uses. The outputs are generated by javac
...
- Android model classes that use Fory type-use annotations such as `@Ref`, `@UInt8Type`, or
  `@Float16Type`.
- Android Fory JSON models that need automatic R8 rules or nested `@JsonCodec` support.
...
exact R8 rules, and type, field, and effective getter `@JsonCodec` declarations remain available
through reflection. Pure type-use codecs require `@JsonType` processor metadata. See
