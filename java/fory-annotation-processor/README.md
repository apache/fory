...
For Android JSON, apply the matching Fory JSON Gradle plugin to every Android application and
Android library module, then annotate each default-mapped model:
...
models require Android Gradle Plugin 8.2 or later, a JDK 17 build with Java 17 source and target
compatibility, and `minSdk 34`. The processor emits exact per-type rule carriers. The Gradle plugin
adds them to application R8 inputs and publishes them as standard AAR consumer rules. Java library
modules use the processor without the Android plugin; their consuming Android application collects
the JAR carriers. Applications must not add a package-wide keep rule. Keep the plugin, runtime, and
processor on the same version.
