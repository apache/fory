                    ...
                    if is_union_compatible && fields_unnamed.unnamed.len() == 1 {
                        ...
                        quote! {
                            Self::#ident(ref value) => {
                                ...
                                value.fory_write(context, true, true, false)?;
                            }
                        }
                    } else {
                        quote! {
                    ...
                    if is_union_compatible && fields_named.named.len() == 1 {
                        ...
                        quote! {
                            Self::#ident { ref #field_ident } => {
                                context.writer.write_varuint32(#tag_value);
                                use fory_core::serializer::Serializer;
                                #field_ident.fory_write(context, true, true, false)?;
                            }
                        }
                    } else {
                        quote! {
    ...
    data_enum
        ...
        .map(|(idx, v)| {
            ...
            match &v.fields {
                ...
                Fields::Unnamed(fields_unnamed) => {
                    if is_union_compatible && fields_unnamed.unnamed.len() == 1 {
                        // Union-compatible single field: read value with ref_info=true, type_info=true
                        let field_ty = &fields_unnamed.unnamed.first().unwrap().ty;
                        quote! {
                            #tag_value => {
                                use fory_core::serializer::Serializer;
                                let value = <#field_ty as Serializer>::fory_read(context, true, true)?;
                                Ok(Self::#ident(value))
                            }
                        }
                    } else {
