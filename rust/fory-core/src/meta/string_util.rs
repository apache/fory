...
#[inline]
fn read_utf16_standard(bytes: &[u8]) -> String {
...
pub fn read_utf16_simd(bytes: &[u8]) -> String {
    assert!(bytes.len() % 2 == 0, "UTF-16 length must be even");
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx2") && bytes.len() >= MIN_DIM_SIZE_AVX {
            return unsafe { read_utf16_simd_avx(bytes) };
        }
    }
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        if is_x86_feature_detected!("sse2") && bytes.len() >= MIN_DIM_SIZE_SIMD {
            return unsafe { read_utf16_simd_sse(bytes) };
        }
    }
    #[cfg(all(target_arch = "aarch64", target_feature = "neon"))]
    {
        if std::arch::is_aarch64_feature_detected!("neon") && bytes.len() >= MIN_DIM_SIZE_SIMD {
            return unsafe { read_utf16_simd_neon(bytes) };
        }
    }
    read_utf16_standard(bytes)
}
