...
/// Test that struct with enum field correctly computes fingerprint with nullable=true for enum fields.
/// This matches Java/C++ behavior where enum fields are always treated as nullable.
#[test]
fn struct_with_enum_field_fingerprint() {
    ...
    struct StructWithEnum {
        name: String,
        color: Color, // This should have nullable=true in fingerprint
        value: i32,
    }
    ...
    // Create a Fory instance with xlang mode and struct version checking
    ...
    // Serialize and deserialize - this should work because the version hash
    // is computed with nullable=true for the enum field
...
/// Test that explicit #[fory(nullable = false)] overrides enum auto-nullable behavior
#[test]
fn struct_with_enum_field_explicit_nullable() {
    ...
    // Struct with explicit nullable=false for enum field
    #[derive(ForyObject, Debug, PartialEq)]
    struct StructWithExplicitNullable {
        name: String,
        #[fory(id = 0, nullable = false)]
        status: Status, // Explicitly non-nullable
    }
