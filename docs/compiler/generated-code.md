...
This document explains the code generated by the Fory IDL compiler for each target language.

## Example Schema

The examples in this document use this Fory IDL schema:
...
Fory IDL unions generate type-safe APIs with an explicit active case. This example is
...
**Note:** Rust uses `Arc` by default for `ref` fields. In Fory IDL, use
...
`ref(weak=true)` in Fory IDL (or `[(fory).weak_ref = true]` in protobuf) to generate
