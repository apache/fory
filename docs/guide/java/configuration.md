...
| `maxGraphMemoryBytes`               | Maximum estimated shallow graph memory accepted during one root deserialization. The default is a fixed `128 MiB`; positive values set an explicit byte limit. Explicit non-positive values are rejected when the runtime is created.                                                                                                                                                                                                                                                                                                         | `134217728`                                                          |
...
- `withMaxGraphMemoryBytes(...)` bounds estimated shallow graph memory during one root
  ...
  workloads need a larger or smaller limit. Explicit non-positive values are rejected when the
  runtime is created.
