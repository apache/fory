...
| `withMaxCachedMemberNames`   | `16K`                                        | Retained common member-name limit; zero disables it    |
...
Depth, concurrency, and retained buffer limits must be positive. The cached-member-name limit must
...
| Builder `IllegalArgumentException` | Use positive depth, concurrency, and retained-buffer values, and a non-negative member-name limit |
